...
path_to_wav = str(EXTERNAL_USE_CASES_DIR / "audio") + "/"
...
def _as_int_list(values):
    return as_int_list(values)


def _format_seconds(values):
    return format_seconds(values)
...
def test_lama(
    ...
    length_in_seconds = motif_length * audio_length_seconds / df.shape[1]
    print(f"Found motif length: {length_in_seconds:.3f}s ({motif_length} samples)")

    for a, eb in enumerate(ml.elbow_points):
        motiflet = np.sort(ml.leitmotifs[eb])
        print(f"Motif {a + 1} positions [s]: {_format_seconds(index_range[motiflet])}")
        print(f"Motif {a + 1} start positions [samples]: {_as_int_list(motiflet)}")
...
def test_mstamp(dataset_name="Star Wars - The Imperial March",
                plot=True, use_mdl=False, write_audio=WRITE_AUDIO):
    get_ds_parameters(dataset_name)
    audio_length_seconds, df, index_range, ground_truth \
        = read_audio_from_dataframe(pandas_file_url, channels)
...
def test_kmotifs(dataset_name="Lord of the Rings Symphony - The Shire",
                 first_dims=True, plot=True, write_audio=WRITE_AUDIO):
    get_ds_parameters(dataset_name)
    audio_length_seconds, df, index_range, ground_truth \
        = read_audio_from_dataframe(pandas_file_url, channels)
...
def test_plot_spectrogram():
    audio_file_urls = \
        [
            TESTS_DIR / "audio" / "snippets" / "Lord of the Rings Symphony - The Shire_Dims_15_Length_301_Motif_1_0.wav",
            TESTS_DIR / "audio" / "snippets" / "Lord of the Rings Symphony - The Shire_Dims_15_Length_301_Motif_1_1.wav",
            TESTS_DIR / "audio" / "snippets" / "Lord of the Rings Symphony - The Shire_Dims_15_Length_301_Motif_1_2.wav",
            TESTS_DIR / "audio" / "snippets" / "Lord of the Rings Symphony - The Shire_Dims_15_Length_301_Motif_1_3.wav"
        ]
...
def main():
    test_publication(plot=False, write_audio=False)
