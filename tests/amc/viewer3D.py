# for motion capturing motif visualization - can be used to make videos
#
...
class Viewer:
  def __init__(self, joints=None, motions=None, start=0, end = -1):
    """
    ...
    self.frame = start # current frame of the motion sequence
    self.start = start
    self.stop = end
  ...
  def run(self):
    ...
    while not self.done:
      ...
      self.draw()
      pygame.display.set_caption(
        'AMC Parser - frame %d / %d' % (self.frame, self.stop)
      )
      pygame.display.flip()
